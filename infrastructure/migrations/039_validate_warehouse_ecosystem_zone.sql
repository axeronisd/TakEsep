-- ==============================================================
-- 039: Validate that warehouse coordinates are inside ecosystem zones
-- ==============================================================

-- 1. Update find_businesses_near to ensure that both customer AND warehouse locations are inside active ecosystem zones
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RETURN; -- Return empty set if customer is outside ecosystem zones
  END IF;
  ...
  WHERE ds.is_active = true
    ...
    AND EXISTS (
      SELECT 1 FROM ecosystem_zones ez
      WHERE ez.is_active = true
        AND is_point_in_polygon(ds.latitude::double precision, ds.longitude::double precision, ez.polygon_points)
    );
END;
$$ LANGUAGE plpgsql STABLE SECURITY DEFINER;


-- 2. Update create_customer_order to restrict orders so that both customer AND warehouse are inside active ecosystem zones
...
DECLARE
  ...
  v_warehouse_zone_found BOOLEAN := false;
...
BEGIN
  ...
  -- ── Check if warehouse is inside any active ecosystem polygon zone ──
  SELECT EXISTS (
    SELECT 1 FROM ecosystem_zones ez
    WHERE ez.is_active = true
      AND is_point_in_polygon(v_pickup_lat::double precision, v_pickup_lng::double precision, ez.polygon_points)
  ) INTO v_warehouse_zone_found;

  IF NOT COALESCE(v_warehouse_zone_found, false) THEN
    RAISE EXCEPTION 'Магазин находится вне зоны доставки экосистемы';
  END IF;
