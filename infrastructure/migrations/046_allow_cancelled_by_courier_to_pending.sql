-- Migration 046 / 20260625002000: Allow transition from cancelled_by_courier to pending and ready, with automatic reset.
...
BEGIN
  ...
  CASE OLD.status
    ...
    WHEN 'cancelled_by_courier' THEN
      IF NEW.status NOT IN ('ready', 'pending', 'cancelled_no_courier') THEN
  ...
  IF NEW.status = 'delivered' THEN
    -- Get dynamic earning rate from couriers table
    SELECT COALESCE(earning_rate, 0.90) INTO v_earning_rate
    FROM couriers
    ...
    ELSIF NEW.delivery_type = 'store' THEN
      -- Store: platform gets organization rate, courier gets 0 (paid offline/salary)
  ...
  -- ═══ Auto-reset cancelled_by_courier back to ready/pending ═══
  IF NEW.status = 'cancelled_by_courier' THEN
    IF NEW.delivery_type = 'store' THEN
      NEW.status := 'ready';
    ELSE
      NEW.status := 'pending';
    END IF;
