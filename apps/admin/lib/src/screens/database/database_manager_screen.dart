...
class _DatabaseManagerScreenState extends State<DatabaseManagerScreen> {
  ...
  static const _tableGroups = <_TableGroup>[
    ...
    _TableGroup('Товары и Каталог', Icons.inventory_2, [
      ...
      _TableDef('store_categories', 'Глобальные категории', Icons.storefront),
      _TableDef('warehouse_store_categories', 'Категории магазинов', Icons.store),
      _TableDef('products', 'Товары', Icons.shopping_bag),
      _TableDef('product_modifier_groups', 'Группы модификаторов', Icons.tune),
      _TableDef('product_modifiers', 'Модификаторы товара', Icons.list),
    ]),
    _TableGroup('Заказы и Доставка', Icons.local_shipping, [
      ...
      _TableDef('delivery_order_item_modifiers', 'Модификаторы в заказах', Icons.playlist_add_check_rounded),
  ...
  Future<void> _deleteSelected() async {
    ...
    try {
      if (_isAuth) {
        ...
        if (_selectedTable == 'delivery_orders') {
  ...
  List<String> _getDependentTables(String table) {
    const deps = <String, List<String>>{
      ...
      'product_modifier_groups': ['product_modifiers', 'delivery_order_item_modifiers'],
      'product_modifiers': ['delivery_order_item_modifiers'],
