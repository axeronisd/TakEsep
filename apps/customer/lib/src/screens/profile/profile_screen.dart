...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void _showEditDataSheet() {
    ...
    final phoneCtrl = TextEditingController(text: _supabase.auth.currentUser?.phone?.replaceFirst('+996', '') ?? '');
  ...
  Future<void> _savePersonalData({required String name, required String username, required String phone}) async {
    final user = _supabase.auth.currentUser;
    if (user == null) return;
    ...
    try {
      ...
      if (cleanPhone.isNotEmpty && currentPhone != '+996$cleanPhone') {
        try {
          ...
          await _supabase.auth.updateUser(UserAttributes(phone: '+996$cleanPhone'));
        } catch (e) {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => _EditPasswordSheet(
        ...
        onSave: () async {
          ...
          if (newPass.length < 6) {
            _snack('Новый пароль должен быть от 6 символов', AkJolTheme.statusPending);
            return;
          }
          ...
          try {
            ...
            await _supabase.auth.signInWithPassword(phone: currentPhone, password: oldPass);
        ...
        actions: [
          TextButton(
            ...
            child: Text('Отмена', style: TextStyle(
              color: _isDark ? const Color(0xFF8B949E) : const Color(0xFF6B7280),
            )),
          ),
          TextButton(
            onPressed: () async {
            ...
            child: const Text('Выйти', style: TextStyle(color: AkJolTheme.error)),
          ),
        ],
      ),
    );
        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              Text('Загрузка профиля...', style: TextStyle(color: muted, fontSize: 14)),
            ],
          ),
        ),
      );
                ...
                delegate: SliverChildListDelegate([
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        _ActionTile(
                          icon: Icons.person_outline, color: AkJolTheme.primary,
                          title: 'Личные данные',
                          subtitle: 'Имя, юзернейм, номер',
                          isDark: _isDark, onTap: _showEditDataSheet,
                        ),
                        Divider(color: border, height: 1, indent: 56),
                        _ActionTile(
                          icon: Icons.lock_outline, color: const Color(0xFF3498DB),
                          title: 'Пароль',
                          subtitle: 'Изменить текущий пароль',
                          isDark: _isDark, onTap: _showEditPasswordSheet,
                        ),
                      ],
                    ),
                  ),
                  ...
                  Container(
                    ...
                    child: _ActionTile(
                      icon: Icons.logout_rounded, color: AkJolTheme.error,
                      title: 'Выйти из аккаунта',
                      subtitle: 'Вы сможете войти снова',
                      isDark: _isDark, onTap: _confirmLogout,
          ...
          child: Column(
            children: [
              Container(
                ...
                child: Center(
                  child: Text(_getInitials(), style: const TextStyle(
              ...
              if (username != null && username.isNotEmpty)
                Padding(
                  ...
                  child: Text('@$username', style: const TextStyle(
                      fontSize: 15, color: AkJolTheme.primary, fontWeight: FontWeight.w600)),
                ),
              ...
              Row(
                ...
                children: [
                  ...
                  Text(user?.phone ?? '', style: TextStyle(fontSize: 14, color: muted)),
                ],
              ),
            ],
          ),
...
class _ActionTile extends StatelessWidget {
  ...
  const _ActionTile({
    ...
    required this.subtitle, required this.isDark, this.onTap,
...
class _EditDataSheet extends StatelessWidget {
  ...
  const _EditDataSheet({
    required this.nameCtrl, required this.usernameCtrl, required this.phoneCtrl,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _sheetField('Юзернейм', usernameCtrl, fieldBg, text, muted, false, isUsername: true),
          const SizedBox(height: 14),
          _sheetField('Номер телефона', phoneCtrl, fieldBg, text, muted, false, isPhone: true),
          const SizedBox(height: 24),
  ...
  Widget _sheetField(String label, TextEditingController ctrl, Color fieldBg,
      Color text, Color muted, bool isObscure, {bool isPhone = false, bool isUsername = false}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        Container(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              prefixIcon: isPhone ? const Padding(
                padding: EdgeInsets.fromLTRB(16, 14, 8, 14),
                child: Text('+996', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
              ) : isUsername ? const Padding(
                padding: EdgeInsets.fromLTRB(16, 14, 8, 14),
...
class _EditPasswordSheet extends StatefulWidget {
  ...
  const _EditPasswordSheet({
    required this.oldPassCtrl, required this.newPassCtrl, required this.confirmPassCtrl,
    required this.isDark, required this.onSave,
  });
...
class _EditPasswordSheetState extends State<_EditPasswordSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    final fieldBg = widget.isDark ? const Color(0xFF21262D) : const Color(0xFFF3F4F6);
    final text = widget.isDark ? Colors.white : const Color(0xFF111827);
    final muted = widget.isDark ? const Color(0xFF8B949E) : const Color(0xFF9CA3AF);
        ...
        children: [
          Center(child: Container(width: 40, height: 4,
            decoration: BoxDecoration(color: muted.withValues(alpha: 0.3),
              borderRadius: BorderRadius.circular(2)))),
          const SizedBox(height: 20),
          Text('Сменить пароль', style: TextStyle(
            fontSize: 20, fontWeight: FontWeight.w800, color: text)),
          const SizedBox(height: 20),

          _passField('Старый пароль', widget.oldPassCtrl, fieldBg, text, muted, _obscureOld, () => setState(() => _obscureOld = !_obscureOld)),
          const SizedBox(height: 14),
          _passField('Новый пароль', widget.newPassCtrl, fieldBg, text, muted, _obscureNew, () => setState(() => _obscureNew = !_obscureNew)),
          const SizedBox(height: 14),
          _passField('Подтвердите пароль', widget.confirmPassCtrl, fieldBg, text, muted, _obscureConfirm, () => setState(() => _obscureConfirm = !_obscureConfirm)),
          const SizedBox(height: 24),
              ...
              child: ElevatedButton(
                onPressed: () {
                  if (widget.newPassCtrl.text != widget.confirmPassCtrl.text) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: const Text('Пароли не совпадают'), backgroundColor: AkJolTheme.error),
                    );
                    return;
                  }
                  widget.onSave();
  ...
  Widget _passField(String label, TextEditingController ctrl, Color fieldBg,
      Color text, Color muted, bool isObscure, VoidCallback toggleEye) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: muted)),
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: IconButton(
                icon: Icon(isObscure ? Icons.visibility_off : Icons.visibility, color: muted, size: 20),
                onPressed: toggleEye,
              ),
            ),
          ),
