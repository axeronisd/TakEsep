...
class _QuickCreateProductDialogState
  ...
  Widget _buildCategoryDropdownWidget(
    ...
    return categoriesAsync.when(
      data: (categories) {
        final dishCatIds = ref.watch(dishCategoryIdsProvider).valueOrNull ?? {};
        final filtered = categories.where((cat) => !dishCatIds.contains(cat.id) || cat.id == _selectedCategoryId).toList();
