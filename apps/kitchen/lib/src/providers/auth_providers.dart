...
const _kLicenseKeyPref = 'takesep_kitchen_license_key';
const _kWarehouseIdPref = 'takesep_kitchen_warehouse_id';
const _kCachedSessionPref = 'takesep_kitchen_cached_session';
const _kBiometricModePref = 'takesep_kitchen_bio_mode'; // 'owner' or 'employee'
const _kBiometricKeyPref = 'takesep_kitchen_bio_key';
const _kBiometricLoginPref = 'takesep_kitchen_bio_login';
const _kBiometricPinPref = 'takesep_kitchen_bio_pin';
...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  Future<bool> loginCompany(String licenseKey) async {
    ...
    if (licenseKey.trim().toUpperCase().startsWith('WH-')) {
      state = state.copyWith(
        isLoading: false,
        error: 'Данный ключ предназначен только для приложения Склад.',
      );
      return false;
    }
    try {
